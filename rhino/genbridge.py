...
def newVector(vector, elevate):
    posX = vector[0] + random.uniform(-1.0, 5.0)
    posY = vector[1] + random.uniform(-1.0, 5.0)
    elevate = vector[2] + elevate + random.uniform(-1, 1)
...
for x in range(6): 
    ...
    if (x > 3): 
        ...
        createPipe(tempPoint, endPointLow, 2.4)
    else: 
        createPipe(tempPoint, startPointLow, 2.4)
